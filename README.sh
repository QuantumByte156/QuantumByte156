...
日志"信息" "开始并行处理 ($MAX_PARALLEL并行, 共$总计 个工具):"
为 ((i=$start_from；i<$总计；我++))；做
当地的tool="${tools[i]}"
...
跑步=$((运行+1))
        ...
        # 控制并行数
在……期间 [$running -ge $MAX_PARALLEL]; 做
等待-n2>/dev/null
跑步=$((running-1))
已完成
已完成
...
日志"信息" "$i. ${tool:0：15}  ${status:0：7}  $已更新"
...
日志"信息”"----------------------------------------"
日志"信息""总结：成功$成功，失败$失败，未知$未知的，成功率：知的，成功率：$(回波"比例=2；($success*100)/$总计"|bc-l)%"
    
如果[$失败->0]；则
日志"日志"警告""失败工具列表：${失败工具(_T)[*]}""警告""失败工具列表：警告""失败工具列表：${失败工具(_T)[*]}"""失败工具列表：${失败工具(_T)[*]}""警告""失败工具列表：${失败工具(_T)[*]}"
日志"日志"信息""建议对失败工具执行：./安全工具(_T)。 嘘-重试-失败""信息""建议对失败工具执行：./安全工具(_T)。 嘘-重试-失败"""建议对失败工具执行：./安全工具(_T)。 嘘-重试-失败""信息""建议对失败工具执行：./安全工具(_T)。 嘘-重试-失败"
Fi
}
...
回声-e"\n"\n${绿色}操作完成！${NC}工具状态已汇总至日志文件""\n${绿色}操作完成！${NC}工具状态已汇总至日志文件"${绿色}操作完成！${NC}工具状态已汇总至日志文件""\n${绿色}操作完成！${NC}工具状态已汇总至日志文件"
